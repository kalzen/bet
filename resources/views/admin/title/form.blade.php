...
@section('content')
@if (isset($record))
<form method="POST" action="{{route('admin.title.update',$record->id)}}">
    @method('PUT')
@else
        ...
        <div class="d-flex align-items-start flex-column flex-md-row">
            <div class="w-100 overflow-auto order-2 order-md-1">
				<div class="card">
                    ...
                    @if(isset($record) && !$record->langParent || !isset($record))
                            <label class="font-weight-semibold">Trang được gắn tiêu đề<span class="required"></span></label>
                            ...
                            <span class="text-muted">Nội dung tiêu dề sẽ được gắn vào (H1) đầu trang (nằm dưới header)</span>
                            @error('route_name')
                                    ...
                                    @else
                                        <li class="list-group-item">Đây là một bài viết phụ của bài viết: <br>
                                            @if(isset($record) && $record->langParent && $record->langParent->langs)
                                                <a href="{{ route('admin.title.edit', $record->langParent->id) }}">{{ $record->langParent->langs->name }}</a>
                                            @else
    ...
    $('#modal_add_lang form').on('submit', function(e) {
        ...
        $('.remove-bind').on('click', function() {
            ...
            swal({
                ...
                if (res.value) {
                    $.ajax({
                        url: '{{ route('admin.title.removeBind', 'idx') }}'.replace(/idx/, id),
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            id: id
        ...
        function goToEdit(id) {
            window.location.href = '{{ route('admin.title.edit', 'idx') }}'.replace(/idx/, id)
        }
