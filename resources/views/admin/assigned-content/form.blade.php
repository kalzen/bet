...
@section('content')
@if (isset($record))
<form method="POST" action="{{route('admin.assigned-content.update',$record->id)}}">
    @method('PUT')
@else
<form method="POST" action="{{route('admin.assigned-content.store')}}">
@endif
    @csrf
    <div class="content">
        <div class="d-flex align-items-start flex-column flex-md-row">
            <div class="w-100 overflow-auto order-2 order-md-1">
				<div class="card">
                    ...
                    @if(isset($record) && !$record->langParent || !isset($record))
                            <label class="font-weight-semibold">Trang được gắn nội dung<span class="required"></span></label>
                            ...
                            <span class="text-muted">Nội dung sẽ được gắn vào cuối trang (nằm trên footer)</span>
                            @error('route_name')
                            <label id="route_name-error" class="validation-invalid-label" for="route_name">{{$message}}</label>
                            @enderror
                    @endif
                        </div>
                ...
                <div class="sidebar-content">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                                    ...
                                    @else
                                        <li class="list-group-item">Đây là một bài viết phụ của bài viết: <br>
                                            @if(isset($record) && $record->langParent && $record->langParent->langs)
                                                <a href="{{ route('admin.assigned-content.edit', $record->langParent->id) }}">{{ $record->langParent->langs->name }}</a>
                                            @else
                            ...
                            @else
                            @if(isset($record))
                                <button class="remove-bind btn btn-sm btn-warning d-none" type="button" data-key="{{ $record->id }}">
                            @endif
...
<div id="modal_add_category" class="modal fade d-none">
        ...
        <div class="modal-content">
            <form action="form_add_lang">
                ...
                <div class="modal-body">
                    ...
                    {{-- <div class="form-group">
                        <label class="font-weight-semibold">Trang được gắn nội dung <span class="required"></span></label>
                        <input type="text" class="form-control maxlength" maxlength="255" required id="route_name" name="route_name" value="{{ old('route_name') ?: ($record->route_name ?? '') }}">
                        <span class="text-muted">Sẽ xuất hiện ở cuối trang này</span>
                        @error('route_name')
                        <label id="route_name-error" class="validation-invalid-label" for="route_name">{{$message}}</label>
                        @enderror
                    </div> --}}
...
//            }
    ...
    $('#modal_add_lang form').on('submit', function(e) {
            ...
            $.ajax({
                url: '{{ route('admin.assigned-content.lang') }}',
                method: 'POST',
        ...
        $('.remove-bind').on('click', function() {
            ...
            swal({
                ...
                if (res.value) {
                    $.ajax({
                        url: '{{ route('admin.assigned-content.removeBind', 'idx') }}'.replace(/idx/, id),
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            id: id
        ...
        function goToEdit(id) {
            window.location.href = '{{ route('admin.assigned-content.edit', 'idx') }}'.replace(/idx/, id)
        }
