...
@section('content')
    ...
    <div class="content">
        <div class="d-flex align-items-start flex-column flex-md-row">
            <div class="w-100 overflow-visible order-2 order-md-1">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <select id="booker_id" name="booker_id" class="selectpicker" style="overflow: show">
                                <option value="">-- Tìm kiếm và chọn Booker --</option>
                                @foreach ($bookers as $booker)
                                    @if (old('booker_id'))
                                        <option value="{{ $booker->id }}"
                                            {{ old('booker_id') == $booker->id ? 'selected' : '' }}>{{ $booker->name }}
                                        </option>
                                    @elseif(isset($record))
                                        <option value="{{ $booker->id }}"
                                            {{ $record->booker->first()->id == $booker->id ? 'selected' : '' }}>
                                            {{ $booker->name }}</option>
                                    @else
