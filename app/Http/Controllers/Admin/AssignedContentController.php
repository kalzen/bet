...
class AssignedContentController extends Controller
{
    protected $routeList;
    ...
    {
        $query = AssignedContent::whereNull('lang_parent_id')->latest();
        $records = $query->paginate();
        $routeList = $this->routeList;
        return view('admin.assigned-content.index', compact('records', 'routeList'));
    }
    ...
    public function create()
    {
        ...
        $modalLangs = $langs;
        $routeList = $this->routeList;
        $query = AssignedContent::whereNull('lang_parent_id')->latest()->get();
        ...
        // return view('admin.assigned-content.form', compact('categories', 'modalLangs', 'formLangs'));
        return view('admin.assigned-content.form', compact('modalLangs', 'formLangs', 'routeList'));
    }

    public function removeBind(Request $request)
    {
        $record = AssignedContent::find($request->id);
    ...
    public function lang(Request $request)
    {
        ...
        $parentPost = AssignedContent::find($request->lang_parent_id);
        ...
        return view('admin.shared.select-lang', [
            'record' => AssignedContent::find($request->lang_parent_id)
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            //     $table->timestamps();
            // });
            // $post = AssignedContent::create($request->only(['title', 'description', 'content', 'status', 'is_promotion', 'keyword', 'lang_id']));
            $routeName = $request->route_name;
            $allPosts = AssignedContent::whereNull('lang_parent_id')->latest();
            ...
            if ($foundPost) {
                DB::rollback();
                return redirect()->route('admin.assigned-content.index')->with('error', $this->routeList[$routeName]['name'] .' đã được gắn bài viết rồi, vui lòng chỉnh sửa nội dung hoặc xoá và tạo lại.');
                dd('ok');
            }
            $post = AssignedContent::create($request->only(['route_name', 'content', 'status', 'lang_id']));
            ...
            // $post->images()->create(['url' => $request->image]);
            DB::commit();
            return redirect()->route('admin.assigned-content.index')->with('message', 'Thêm mới thành công');
        } catch (Exception $ex) {
            DB::rollback();
    ...
    public function edit($id)
    {
        // $langs = Lang::all();
        // $categories = Category::query()->whereNull('parent_id')->whereNull('lang_parent_id')->orderBy('name', 'asc')->get();
        $record = AssignedContent::find($id);
        if($record == null){
            return redirect()->route('admin.assigned-content.index');
        }
        $formLangs = SharedHelper::getExcludedFormLangs($record);
        $modalLangs = SharedHelper::getExcludedModalLangs($record);
        $routeList = $this->routeList;
        $query = AssignedContent::whereNull('lang_parent_id')->where('route_name', '!=', $record->route_name)->latest()->get();
        $routeList = array_diff_key($routeList, array_flip($query->pluck('route_name')->toArray()));
        ...
        return view('admin.assigned-content.form', compact('record', 'success', 'issue', 'inlinks', 'formLangs','modalLangs', 'routeList'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('admin.assigned-content.index')->with('message', 'Cập nhật thành công');
        } catch (Exception $ex) {
            DB::rollback();
            return back()->withInput();
        }
    ...
    public function destroy($id)
    {
        $record = AssignedContent::find($id);
        if ($record) {
            AssignedContent::where('lang_parent_id', $id)->delete();
            $record->delete();
        }
